...
def upgrade() -> None:
    ...
    brand_voice_style = postgresql.ENUM("expert", "community", "story", "data", name="brand_voice_style")
    platform_type = postgresql.ENUM("linkedin", "twitter", "instagram", "facebook", name="platform_type")
    post_platform_type = postgresql.ENUM("linkedin", "twitter", "instagram", "facebook", name="post_platform_type")
    performance_platform_type = postgresql.ENUM("linkedin", "twitter", "instagram", "facebook", name="performance_platform_type")
    post_status = postgresql.ENUM("draft", "scheduled", "published", "failed", "paused", name="post_status")
    ...
    calendar_status = postgresql.ENUM("draft", "approved", "live", name="calendar_status")
    approval_mode = postgresql.ENUM("full_review", "spot_check", "autonomous", name="approval_mode")
    job_type = postgresql.ENUM("single_post", "weekly_calendar", "month_calendar", name="job_type")
    ...
    op.create_table(
        ...
        sa.Column("style", brand_voice_style, nullable=False),
        sa.Column("persona_prompt", sa.Text(), nullable=False),
        sa.Column("sample_approved_posts", postgresql.JSONB(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
    ...
    op.create_index("ix_content_calendar_company_id", "content_calendar", ["company_id"])
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False, server_default=sa.text("gen_random_uuid()")),
        sa.Column("company_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("companies.id"), nullable=False, unique=True),
        sa.Column("mode", approval_mode, nullable=False, server_default="full_review"),
        sa.Column("reviewer_user_ids", postgresql.JSONB(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
    )
