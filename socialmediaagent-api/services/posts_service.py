...
def _sync_status_from_schedule(post: Post) -> None:
    """Keep draft/scheduled aligned with scheduled_at. Do not override terminal states."""
    if post.status in (PostStatus.published, PostStatus.failed, PostStatus.expired):
        return
...
async def _require_active_platform_connection(
    db: AsyncSession, company_id, platform: PlatformType
) -> None:
    ...
    if result.scalar_one_or_none() is None:
        raise ValueError(
            f"Connect {platform.value} under Platforms before creating a post for it."
        )


async def upload_post_media(
    file: UploadFile, request: Request, db: AsyncSession, user: User
) -> dict[str, Any]:
    ...
    if settings.CLOUDINARY_CLOUD_NAME:
        ...
        result = await upload_to_cloudinary(raw, filename, resource_type)
...
async def list_posts(pagination, db: AsyncSession, user: User):
    ...
    return paginated_data(
        [_post_payload(p) for p in result.scalars().all()],
        total=total,
...
async def create_post(payload, db: AsyncSession, user: User):
    ...
    await publish_service.try_publish_post_by_id(
        db, str(post.id), force_immediate=False
    )
    await db.refresh(post)
    return _post_payload(post)
...
async def update_post(post_id: str, payload, db: AsyncSession, user: User):
    ...
    if post.status not in (PostStatus.published, PostStatus.failed, PostStatus.expired):
        _sync_status_from_schedule(post)
    await db.commit()
    await db.refresh(post)
    settings = get_settings()
    if (
        settings.POSTFORME_API_KEY.strip()
        and post.status == PostStatus.scheduled
        and not post.external_publish_id
    ):
        await publish_service.try_publish_post_by_id(
            db, str(post.id), force_immediate=False
        )
...
async def generate_post(
    payload, background_tasks: BackgroundTasks, db: AsyncSession, user: User
):
    company = await get_company_for_user(db, user)
    if company is None:
...
async def get_post_performance(post_id: str, db: AsyncSession, user: User):
    ...
    result = await db.execute(
        ...
        .join(Post, and_(Post.id == PostPerformance.post_id))
        .where(Post.id == post_id, Post.company_id == company.id)
    )
