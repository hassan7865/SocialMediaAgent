...
async def get_brand_voice(db: AsyncSession, user: User):
    ...
    if company is None:
        return None
    result = await db.execute(select(BrandVoice).where(BrandVoice.company_id == company.id))
    return _brand_voice_payload(result.scalar_one_or_none())


async def analyze_brand_voice(tone_samples: list[str], background_tasks: BackgroundTasks):
    background_tasks.add_task(lambda: None)
    return {"tone_samples": tone_samples, "analysis": {"style": "expert", "formality_level": 3}}
...
async def save_brand_voice(payload, db: AsyncSession, user: User):
    ...
    if brand_voice:
        ...
        brand_voice = BrandVoice(company_id=company.id, **payload.model_dump(exclude_none=True))
        db.add(brand_voice)
    await db.commit()
